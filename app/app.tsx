...
import { fetchAppParams, fetchChatList, generationConversationName, sendChatMessage, updateFeedback } from '@/service'
...
const App: FC<AppProps> = () => {
  ...
  const conversationIntroduction = ''

  const getCurrConversationId = () => {
    return '-1'
  }
  ...
  */
  ...
  // init
  // This log is called twice because:
  // 1. Initial render in strict mode
  // 2. When serverConfig actually changes
  console.log('init===serverConfig')
  useEffect(() => {
  ...
  const handleSend = async (message: string, files?: VisionFile[]) => {
    ...
    const data: Record<string, any> = {
      inputs: {}, //currInputs,
      query: message,
      conversation_id: isNewConversation ? null : -1//currConversationId,
    }
    ...
    sendChatMessage(data, {
      ...
      async onCompleted(hasError?: boolean) {
        ...
        if (getConversationIdChangeBecauseOfNew()) {
          ...
          const newItem: any = await generationConversationName(conversations[0].id)
